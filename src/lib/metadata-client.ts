...
/**
 * Extract metadata from an EPUB file (client-side)
 * @param fileBuffer - The EPUB file as an ArrayBuffer
 * @returns Extracted metadata
 */
export async function extractEpubMetadata(fileBuffer: ArrayBuffer): Promise<MediaMetadata> {
  try {
...
/**
 * Extract metadata from a PDF file (client-side)
 * @param fileBuffer - The PDF file as an ArrayBuffer
 * @returns Extracted metadata
 */
export async function extractPdfMetadata(fileBuffer: ArrayBuffer): Promise<MediaMetadata> {
  try {
...
/**
 * Extract metadata from a file based on its type (client-side)
 * @param file - The file to extract metadata from
 * @returns Extracted metadata
 */
export async function extractMetadataClient(file: File): Promise<MediaMetadata> {
  const arrayBuffer = await file.arrayBuffer()
  
  // Determine file type from MIME type or extension
  const fileType = file.type === 'application/epub+zip' ? 'epub' : 
